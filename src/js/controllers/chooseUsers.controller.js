    ...
    function ChooseUsersController($scope, $state, $stateParams, $rootScope, $ionicLoading, publicService, $ionicHistory) {
        ...
        function saveBtnClick() {
            var params = {
                ...
                'memberIds.s': vm.memberIds,
                'memberNames.s': vm.memberNames,
                'memberLoginNames.s': vm.memberLoginNames,
                'purpose.s': vm.meetingTitle,
                'leadTime.s': vm.noticeSelect,
                'startTime.s': $stateParams.sTime,
                'endTime.s':$stateParams.eTime
            };
            ...
            publicService.sendRequest('apply', params, function(msg) {
                var code = msg[0].h[0]['code.i'];
                if (code == 0) {
                    vm.result = msg[1].b[0]['value.b'];
                    if (vm.result == '1') {
                        vm.isDetail = false;
                        vm.dTitle = '预约详情';
                        //保存成功后返回list页面
                    ...
                    if (code == 84001) {
                        $ionicLoading.show({
                            template: '<span class="tips">预约失败，时间冲突</span>',
                            noBackdrop: true,
                            duration: 2000
                        });
                    } else {
                        $ionicLoading.show({
                            template: '<span class="tips">预约失败，请重新预约时间</span>',
                            noBackdrop: true,
                            duration: 2000
                        });
                    }
